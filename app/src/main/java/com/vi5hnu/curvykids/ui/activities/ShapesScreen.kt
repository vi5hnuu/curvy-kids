...
private fun DrawScope.drawShape(id: String, color: Color) {
    ...
    when (id) {
        ...
        "square" -> drawRoundRect(color, topLeft = Offset(w * 0.14f, h * 0.14f), size = Size(w * 0.72f, h * 0.72f), cornerRadius = androidx.compose.ui.geometry.CornerRadius(w * 0.12f))
        "triangle" -> {
            val path = Path().apply {
